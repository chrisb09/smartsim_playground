...
if [[ $PROVIDER == "SMARTSIM" ]]; then
	...
	echo "Using PHYDLL_DL_CLIENT=${DL_CLIENT_CMD[*]}"
	...
	"${MPIRUN_BIN}" "${PHY_APP_ENV[@]}" -n "${NP_PHY}" "${SCRIPT_DIR}/build/module_test_solver" "${CONFIG_FILE}" : "${DL_APP_ENV[@]}" -n "${NP_DL}" "${DL_CLIENT_CMD[@]}"
else
