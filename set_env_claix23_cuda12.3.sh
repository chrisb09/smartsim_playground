...
# Apparently, cuda 12.3 is not compatible with gcc 13, so we have to use gcc 11.3 for this configuration.
# https://forums.developer.nvidia.com/t/nvcc-12-3-with-gcc-13-2-not-working/271476/6
...
if [ "$any_load_required" = true ]; then
    echo "Purging and loading required modules for SmartSim (CUDA11 + MPI + HDF5)..."
    module purge
